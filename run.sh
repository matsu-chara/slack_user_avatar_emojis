...
curl --silent --show-error "https://slack.com/api/users.list?token=$SLACK_API_TOKEN" | jq -r '.members | map(select(.is_bot == false)) | map(.name + "\t" + .profile.image_72)[]' | while read -r name avatar
do
